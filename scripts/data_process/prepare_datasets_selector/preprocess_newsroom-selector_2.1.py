...
output_file = '../recsum_/data/newsroom/selector/%s-selector-2.1.json'
title_file = '../recsum_/data/newsroom/synthesized_user/%s-titles.json'
...
split = 'dev'
if not os.path.exists("../recsum_/data/newsroom/selector/%s-selector-2.1/" % split):
    os.mkdir("../recsum_/data/newsroom/selector/%s-selector-2.1/" % split)
...
for fold in range(num_folds):
    ...
    for result in results:
        for k, v in result.items():
            results_ += v
    with open("../recsum_/data/newsroom/selector/%s-selector-2.1/21-%s.json" % (split, fold), 'w') as f:
...
"""
con = []
fnames = [fname for fname in os.listdir("../recsum_/data/newsroom/selector/%s-selector-2.1/" % split) if
          not fname.startswith('train-selector')]

progress = tqdm(range(len(fnames)), desc='loading files')
for fname in fnames:
    with open("../recsum_/data/newsroom/selector/%s-selector-2.1/" % split + fname) as f:
        con_ = json.load(f)
    con = con + con_
    _ = progress.update(1)

progress = tqdm(range(len(con)), desc='writing data')
with jsonlines.open("../recsum_/data/newsroom/selector/%s-selector-2.1.json" % split, 'w') as f:
    for line in con:
        _ = f.write(line)
        _ = progress.update(1)
